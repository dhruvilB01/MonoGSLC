...
def create_fast_brief(nfeatures: int = 2000):
    ...
    if not hasattr(cv2, "xfeatures2d"):
        raise SystemExit(
            "cv2.xfeatures2d.BriefDescriptorExtractor_create is required. Install opencv-contrib-python."
        )
    brief = cv2.xfeatures2d.BriefDescriptorExtractor_create(bytes=32)
    return fast, brief


def extract_fast_brief(
    fast: cv2.FastFeatureDetector, brief, img_gray: np.ndarray, max_kpts: int
) -> Tuple[List[cv2.KeyPoint], np.ndarray]:
    kps = fast.detect(img_gray, None)
    # Keep top responses for speed
    kps = sorted(kps, key=lambda k: -k.response)[:max_kpts]
    kps, desc = brief.compute(img_gray, kps)
    if desc is None:
        desc = np.zeros((0, brief.descriptorSize()), dtype=np.uint8)
    return kps, desc


def create_matcher() -> cv2.DescriptorMatcher:
    # BRIEF is binary -> use Hamming
...
def detect_loops_online(
    ...
    fast, brief = create_fast_brief(nfeatures=max_kpts)
    ...
    for i, path in enumerate(frames):
        ...
        kp_i, desc_i = extract_fast_brief(fast, brief, gray, max_kpts=max_kpts)
...
def main():
    ap = argparse.ArgumentParser(description="Online FAST+BRIEF loop detection over a frames directory")
    ...
    ap.add_argument("--max_kpts", type=int, default=2000, help="Max FAST keypoints per frame")
